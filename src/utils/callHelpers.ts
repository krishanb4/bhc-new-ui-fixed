...
export const stakeBHC = async (masterChefContract, amount, account) => {
  return masterChefContract.methods
...
export const unstakeBHC = async (masterChefContract, amount, account) => {
  return masterChefContract.methods
    .withdrawAmount(new BigNumber(amount).times(new BigNumber(10).pow(18)).toString())
    .send({ from: account })
    .on('transactionHash', (tx) => {
      return tx.transactionHash
    })
...
export const harvestBHC = async (farmContract, account) => {
